...
var hola = new ListBuffer[String]()

var cad2:String = cadena.replace(" ","")
var letra:String = ""
var numMay:Int = 0
var indice:Int = -1

for(k <- 0 to cad2.length-1)
{ 
	if (cad2(k).isUpper)
	{ 
		numMay +=1 
...
else 
{
	...
	{ 
		var pal:String = ""
		letra = cad2(i).toUpper.toString
		for(j <- 0 to cad2.length-1){
		if (i == j) {pal = pal + letra} 
		else {pal = pal + cad2(j).toString}
		}
		if (i != indice) {hola += pal}
	}
...
var Arre:Array [Any] = Array("Error")
var c = new ListBuffer[Int]()
var ent = new ListBuffer[Any]()

if(lis.isEmpty== false)
{
	for(x <- 0 to lis.length-1)
	{
		if(lis(x)== 0) c += 0
		else ent = ent += lis(x)
	}
	Arre = List.concat(c,ent).toArray
}
...
def ejercicio4(parentesis:String):Boolean = 
{
	...
	def eje4(prntss: List[Char], Nparent: Int): Int = 
	{
	...
	if (eje4(parentesis.toList, 0) == 0 ) {true}
	else {false}
...
{
	import scala.collection.mutable.ListBuffer
	var r = new ListBuffer[List[Double]]()
	var t = new ListBuffer[Double]()
	var c = 0.0

	for(k <- 0 to m1.length-1)
	{
		t.clear
		for(y <- 0 to m2(0).length-1)
		{
			c = 0.0
			for(x <- 0 to m2.length-1)
			{
				c += m1(k)(x)* m2(x)(y)
			}
			t += c
...
{
	def prim(n: Stream[Int]): Stream[Int] = 
	{
		n.head #:: prim(n.tail.filter(_ % n.head != 0))
	}

	val lista = prim(Stream.from(2)).take(n).toList
...
def ejercicio8(pangrama: String): Boolean = 
{
	...
	var oracion = pangrama.toLowerCase.replace('á','a').replace('é','e').replace('í','i').replace('ó','o').replace('ú','u').replace(" ","").sorted
	var oracion2 = oracion.toList
	var i = 0
	var bolean = true

	if (oracion.length-1 == 25 & alfa == oracion2)
	{
		bolean = false
	} 
	else
	{
		while (bolean && i <= 25)
		{
			var cuenta = oracion.count(_ == alfa(i))
			if (cuenta > 0) 
			{
				bolean = true
			}
...
*/
def ejercicio9(num:String): String = 
{ 
	var m = Map("cero" -> 0, "uno" -> 1, "dos" -> 2, "tres" -> 3, "cuatro" -> 4, "cinco" -> 5, "seis" -> 6, "siete" -> 7, "ocho" -> 8, "nueve" -> 9)
	var m1 = Map (0 -> "cero", 1 -> "uno", 2 -> "dos", 3 -> "tres", 4 -> "cuatro", 5 -> "cinco", 6 -> "seis", 7 -> "siete", 8 -> "ocho", 9 -> "nueve")
	var r = ""
	var sum = 0
...
ejercicio10("","") = "ERROR"
*/
def ejercicio10(ent:String,clave:String): String=
{
	val cif = Array("a","b","c","d","e","f","g","h","i","j","k","l","m","n","o","p","q","r","s","t","u","v","w","x","y","z")
	var ent1 = ent.toLowerCase
	var clavel = clave.toLowerCase
	var r = ""
	var i=0
	var j = 0
	if ((ent.length > 0) && (clave.length > 0)) 
	{
		for (i<-0 to ent1.length()-1) 
		{
			if (i <= clavel.length-1)
			{
				r += cif((cif.indexOf(ent1(i).toString) + cif.indexOf(clavel(i).toString) ) % 26)
			}
			else 
			{
				r += cif((cif.indexOf(ent1(i).toString) + cif.indexOf(clavel(j).toString) ) % 26)
				j +=1
			}
		}
	} else {r = "error"}
